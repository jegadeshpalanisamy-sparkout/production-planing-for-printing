                        ...
                        @foreach ($processes as $process)
                            <tr class="bg-gray-100 text-black">
                                ...
                                <td class="border px-4 py-2">
                                    <a href="{{ route('admin.edit_process', $process->id) }}"
                                        class="text-white text-decoration-none">
                                        <button class="bg-blue-500 text-white px-3 py-2 rounded hover:bg-blue-600">
                                            Edit
                                        </button>
                                    </a>
                                    <form action="{{ route('admin.delete_process', $process->id) }}" method="POST"
                                        class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="bg-red-500 text-white px-3 py-2 rounded hover:bg-red-600">Delete</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
