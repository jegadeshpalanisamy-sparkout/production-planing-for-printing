                    ...
                    <div class="max-w-lg mx-auto bg-white p-8 rounded-lg shadow-lg">
                        ...
                        <form action="{{ route('employees.assign_order') }}" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <select id="order_select" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" name="order_id" required>
                                    <option value="">-- Select Order --</option>
                                    @foreach ($processes as $process)
                                        <option value="{{ $process->order_id }}">{{ $process->order->order_number }}</option>
                                    @endforeach
                                </select>
                                @error('order_id')
                                    <div class="text-red-500">{{ $message }}</div>
                            ...
                            <div id="processes_section" class="mt-2">
                                ...
                                @foreach ($processes as $process)
                                    <div>
                                        <input type="checkbox" name="process_id[]" value="{{ $process->process_id }}" id="process_{{ $process->process_id }}" class="mr-2">
                                        <label for="process_{{ $process->process_id }}" class="text-sm">{{ $process->process->name }}</label>
                                    </div>
                                @endforeach
                                @error('process_id')
                                    <div class="text-red-500">{{ $message }}</div>
                            ...
                            <div class="mb-4 mt-2">
                                ...
                                <select id="employee_select" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" name="employee_id" required>
                                    <option value="">-- Select Employee --</option>
                                    @foreach ($employees as $employee)
                                        <option value="{{ $employee->id }}">{{ $employee->emp_name }}</option>
                                    @endforeach
                            ...
                            <!-- Submit and Back Buttons -->
                            <div class="mt-6 gap-2 flex">
                                <button type="submit" class="bg-green-500 hover:bg-green-600 text-white font-bold p-2 rounded">Assign</button>
                                <a href="" class="bg-violet-500 hover:bg-violet-600 text-white font-bold py-2 px-4 rounded">Back</a>
                            </div>
                        </form>
                    </div>
                </div>
